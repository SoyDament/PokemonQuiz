document.addEventListener("DOMContentLoaded", function () {
    ...
    function getRandomPokemonId() {
        return Math.floor(Math.random() * 135) + 252; // Pokémon de la tercera generación
    }
    ...
    function getAllThirdGenPokemonNames(callback) {
        const xhr = new XMLHttpRequest();
        xhr.open("GET", "https://pokeapi.co/api/v2/pokemon?limit=135&offset=251", true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                const data = JSON.parse(xhr.responseText);
    ...
    function generateOptions(correctName, callback) {
        getAllThirdGenPokemonNames((allThirdGenPokemonNames) => {
            const options = [];

            while (options.length < 3) {
                const randomIndex = Math.floor(Math.random() * allThirdGenPokemonNames.length);
                const randomName = allThirdGenPokemonNames[randomIndex];
    ...
    function startGame() {
        ...
        pokemonImage.style.animation = "reveal 2s ease-in-out";

        setTimeout(() => {
            getRandomPokemon((data) => {
                pokemonImage.src = data.pngUrl;
